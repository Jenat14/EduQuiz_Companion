...
const LeadView = () => {
  ...
  //fetching quiz details and questions
  useEffect(() => {
    const fetchQuizDetailsAndQuestions = async () => {
      try {
        ...
        const quizResponse = await fetch(
          `http://localhost:3000/quizdata/quizd?subjectId=${id}&level=${level}&name=${parsedQuizData}`
        );
        if (!quizResponse.ok) {
        ...
        const questionsResponse = await fetch(
          "http://localhost:3000/quizdata/questions",
          {
            method: "POST",
            headers: {
  ...
  //chatbot messages sending and receiving
  ...
  return (
    <>
      <div className="page" style={{ marginTop: "70px" }}>
        <div
          className="top-bar bg-teal text-white py-2"
          style={{ height: "150px" }}
        >
          <div className="container-fluid">
            ...
            >
              ...
              {role === "faculty" ? (
                <Link
                  to={`/Leaderboard?quizId=${localStorage.getItem("quizid")}`}
                >
        ...
        <div>
          <div style={{ paddingTop: "30px" }}>
            Time Limit: {quizDetails.time} min
          </div>
          <div style={rightAlign}>
            <div style={{ paddingRight: "30px" }}>
              Total Questions: {quizDetails.numberOfQuestions}
            </div>
            <div style={{ paddingRight: "30px" }}>
              Maximum Mark: {quizDetails.totalMarks}
            </div>
          </div>
        </div>
        {/* Main Quiz Section */}
                  ...
                  {[1, 2, 3, 4].map((optionIndex) => {
                    ...
                    return (
                      <div key={optionIndex} className="col-6 option">
                        <label
                          htmlFor={`q${index + 1}_option${optionIndex}`}
                          style={labelStyle}
                        >
                          <input
                            type="radio"
                          ...
                          />
                          {/* Checking if option matches selectedOption */}
