...
const Question = () => {
  ...
  //fetching the quiz details and questions from backend
  useEffect(() => {
  ...
  //prevent the user from pressing back and coming to this page
  useEffect(() => {
    const preventBack = () => {
  ...
  //print an alert on changing tab
  useEffect(() => {
  ...
  const handleFinish = async () => {
    ...
    questions.forEach((question, index) => {
      ...
      if (selectedOptionIndex !== undefined && selectedOptionIndex !== null) {
        attemptedQuestions++;
        if (
          question[`option${selectedOptionIndex + 1}`] ===
    ...
    try {
      const response = await fetch(
        "http://localhost:3000/resultUpdatedRoutes",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
